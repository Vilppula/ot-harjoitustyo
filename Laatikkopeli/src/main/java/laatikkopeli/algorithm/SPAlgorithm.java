    ...
    public int count(char[][] r, int gI, int gJ) {                              //Move player and calculate shortest path (allowance have been checked ex-SPA)
        ...
        while (!queue.isEmpty()) {
            ...
            for (int m = 0; m < 4; m++) {                                       //Have to four directions from this state
                ...
                if (newY < 0 || newY >= h || newX < 0 || newX >= w) {           //If move is directed outside gamearea, just skip current try (discard new possible state)
                    continue;
                }   
                if (usedStates [newY][newX][now[2]][now[3]] != 0) {             //If this new state is already been checked, skip
                    continue;
                }     
                char c = r[newY][newX];                                         //Read the char in new location (player)
                if (c == '#') {                                                 //If that char stands for wall, just skip the try
                    continue;
                }                                                               
                ...
                if (newY == now[2] && newX == now[3]) {                         //If it is box, check next if box is allowed to move
                    ...
                    if (newBY < 0 || newBX < 0
                            || newBY >= h || newBX >= w) {                      //Skip states that would push box out of the gamearea
                        continue;
                    }              
                    char cc = r[newBY][newBX];                                  //Check the char in new postion (box)
                    if (cc == '#') {                                            //If that char is wall, skip the try
                        continue;           
                    }                                    
                    if (newBY == gI && newBX == gJ) {                           //If that char marks as the goal of player 1, return the distance to that state
                        return dist + 1;
                    }            
